...
{capture name="label" assign="label"}{sugar_translate label='LBL_PHONE_OFFICE_2' module='Accounts'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{if strlen($fields.phone_office_2_c.value) <= 0}
{assign var="value" value=$fields.phone_office_2_c.default_value }
{else}
{assign var="value" value=$fields.phone_office_2_c.value }
{/if}  
<input type='text' name='{$fields.phone_office_2_c.name}' id='{$fields.phone_office_2_c.name}' size='30' maxlength='255' value='{$value}' title='' tabindex='0'	  class="phone" >
<td valign="top" id='phone_office_3_c_label' width='12.5%' scope="col">
{capture name="label" assign="label"}{sugar_translate label='LBL_PHONE_OFFICE_3' module='Accounts'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{if strlen($fields.phone_office_3_c.value) <= 0}
{assign var="value" value=$fields.phone_office_3_c.default_value }
{else}
{assign var="value" value=$fields.phone_office_3_c.value }
{/if}  
<input type='text' name='{$fields.phone_office_3_c.name}' id='{$fields.phone_office_3_c.name}' size='30' maxlength='255' value='{$value}' title='' tabindex='0'	  class="phone" >
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_ACCOUNT_SOURCE' module='Accounts'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.account_source.name}" 
id="{$fields.account_source.name}" 
title=''       
>
{if isset($fields.account_source.value) && $fields.account_source.value != ''}
{html_options options=$fields.account_source.options selected=$fields.account_source.value}
{else}
{html_options options=$fields.account_source.options selected=$fields.account_source.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.account_source.options }
{capture name="field_val"}{$fields.account_source.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.account_source.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.account_source.name}" 
id="{$fields.account_source.name}" 
title=''          
>
{if isset($fields.account_source.value) && $fields.account_source.value != ''}
...
<input
id="{$fields.account_source.name}-input"
name="{$fields.account_source.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.account_source.name}-image"></button><button type="button"
id="btn-clear-{$fields.account_source.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.account_source.name}-input', '{$fields.account_source.name}');sync_{$fields.account_source.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
...
{literal}
(function (){
var selectElem = document.getElementById("{/literal}{$fields.account_source.name}{literal}");
if (typeof select_defaults =="undefined")
select_defaults = [];
select_defaults[selectElem.id] = {key:selectElem.value,text:''};
//get default
...
YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters", "node","node-event-simulate", function (Y) {
{/literal}
SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.account_source.name}-input');
SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.account_source.name}-image');
SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.account_source.name}');
{literal}
function SyncToHidden(selectme){
var selectElem = document.getElementById("{/literal}{$fields.account_source.name}{literal}");
var doSimulateChange = false;
...
//global variable 
sync_{/literal}{$fields.account_source.name}{literal} = function(){
SyncToHidden();
}
function syncFromHiddenToWidget(){
var selectElem = document.getElementById("{/literal}{$fields.account_source.name}{literal}");
...
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.simulate('keyup');
for (i=0;i<selectElem.options.length;i++){
if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.account_source.name}-input{literal}'))
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
}
}
YAHOO.util.Event.onAvailable("{/literal}{$fields.account_source.name}{literal}", syncFromHiddenToWidget);
{/literal}
SUGAR.AutoComplete.{$ac_key}.minQLen = 0;
SUGAR.AutoComplete.{$ac_key}.queryDelay = 0;
SUGAR.AutoComplete.{$ac_key}.numOptions = {$field_options|@count};
...
addToValidate('EditView', 'account_source', 'enum', false,'{/literal}{sugar_translate label='LBL_ACCOUNT_SOURCE' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'phone_office_2_c', 'phone', false,'{/literal}{sugar_translate label='LBL_PHONE_OFFICE_2' module='Accounts' for_js=true}{literal}' );
addToValidate('EditView', 'phone_office_3_c', 'phone', false,'{/literal}{sugar_translate label='LBL_PHONE_OFFICE_3' module='Accounts' for_js=true}{literal}' );
