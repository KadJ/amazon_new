    ...
    */
	...
	function sendEmail($mail,$save_emails=1,$testmode=false){
        ...
        if ((!isset($module->email_opt_out)
            ...
            $GLOBALS['log']->fatal("********************");
            $GLOBALS['log']->fatal($this->marketing_id);
			//fetch email template associate with the marketing message.




			if (empty($this->current_emailtemplate) or $this->current_emailtemplate->id !== $this->current_emailmarketing->template_id OR true) {
				if (!class_exists('EmailTemplate')) {
                ...
                /**
                 * Прикрепляем PDF-документ из шаблона
                 */
                $pdf_notes_remove = array();
                $GLOBALS['log']->fatal('$this->current_emailtemplate->pdf_template_id=' . $this->current_emailtemplate->pdf_template_id);
                if($this->current_emailtemplate->pdf_template_id) {
                    // Если в шаблоне указан PDF-шаблон прикрепляемого документа
                    $seedPDFTemplate = new AOS_PDF_Templates();
                    $seedPDFTemplate->retrieve($this->current_emailtemplate->pdf_template_id);

                    $object_arr = array();

                    $object_arr[$module->module_dir] = $module->id;
                    ...
                    if(isset($GLOBALS['for_pdf_current_emailmarketing'][$GLOBALS['for_pdf_current_emailmarketing_id']])) {
                        $text = str_replace('$email_counter', $GLOBALS['for_pdf_current_emailmarketing'][$GLOBALS['for_pdf_current_emailmarketing_id']]->start_email_outbound_conter, $text);
                        $GLOBALS['for_pdf_current_emailmarketing'][$GLOBALS['for_pdf_current_emailmarketing_id']]->start_email_outbound_conter++;
                    }
                    ...
                    $file_name = "test.pdf";
                        ...
                        ob_clean();
                        try{
                            // Создаем заметку с PDF
                            $seedNote = new Note();
                            $seedNote->name = $seedPDFTemplate->name . '.pdf';
                            $seedNote->filename = $seedPDFTemplate->name . '.pdf';
                            $seedNote->file_mime_type = 'application/pdf';
                            $seedNote->parent_type = 'Emails';
                            $seedNote->parent_id = $this->current_emailmarketing->template_id;
                            $seedNote->save();
                            $pdf_notes_remove[] = $seedNote;
                            $this->notes_array[] = $seedNote;
                            ...
                            fclose($fp);
                            $pdf->Output($sugar_config['upload_dir'] . $seedNote->id,'F');
            ...
            if(!empty($module->email2)) {
                $mail->AddAddress($module->email2,$locale->translateCharsetMIME(trim($module->name), 'UTF-8', $OBCharset) );
            }
            //$mail->AddAddress('uspensky@lemars.ru',$locale->translateCharsetMIME(trim($module->name), 'UTF-8', $OBCharset) );
            //$mail->AddAddress('marina@egovexpert.ru',$locale->translateCharsetMIME(trim($module->name), 'UTF-8', $OBCharset) );
            ...
            if($this->current_emailmarketing->custom_email_enabled) {
                // Если стоит своя настройка почтового сервера
                $mail->mail_sendtype = 'SMTP';
                $mail->Mailer = 'smtp';
                $mail->Host = $this->current_emailmarketing->custom_email_smtp_server;
                $mail->Port = $this->current_emailmarketing->custom_email_smtp_port;
                if ($this->current_emailmarketing->custom_email_protocol == 1) {
                    $mail->SMTPSecure = 'ssl';
                } // if
                if ($this->current_emailmarketing->custom_email_protocol == 2) {
                    $mail->SMTPSecure = 'tls';
                } // if
                $mail->SMTPAuth = TRUE;
                $mail->Username = $this->current_emailmarketing->custom_email_username;
                $mail->Password = $this->current_emailmarketing->custom_email_password;

                $GLOBALS['log']->fatal("Установлены данные");
                $GLOBALS['log']->fatal($mail->Username);
        ...
        /**
         * Удаление вложений с генерированными PDF-документами
         */
        if(isset($pdf_notes_remove)) {
            foreach($pdf_notes_remove as $seedNote) {
                $seedNote->mark_deleted($seedNote->id);
                unlink($sugar_config['upload_dir'] . $seedNote->id);
            }
        }
